...
history:
  - v1: Initial design
  - v2: Post-validation errata. The §Elevation ladder table was in CIE L* but labelled its
        column "luminance", and the frozen test measures WCAG Y — the two are not
        interchangeable, and D7's stated "+2-3 L*" computes to less than the test's own floor.
        The table also mislabelled `element.background` as `surface.background` and reported
        the dead step as "+0.2" when it is exactly 0.0000. Adds "Borders travel with the
        ladder", which records the regression that 5.1 shipped and R2.1 failed to catch.
---
...
> **ERRATUM (v2) — this table was wrong twice, and the second error nearly sank the fix.**
> The column below was labelled "luminance" but the numbers are **CIE L\***, while the test
> that gates this ladder measures **WCAG relative luminance (Y)**. The two are *not*
> interchangeable, and the conversion is non-linear: at the dark end **+4 L\* is only
> +0.0099 Y** — below the test's own `MIN_LUMINANCE_STEP` floor of **0.01 Y** — whereas at
> the light end +3 L\* is +0.07 Y. **Sizing the rungs to D7's literal "+2-3 L\*" would have
> failed the very test D7 exists to satisfy.** The rule that falls out: *specify the ladder
> in the unit you test it in.* This table now carries both, and the normative column is Y.
> The table also mislabelled `#2e343e` as `surface.background` — that hex is
> `element.background`. The real `surface.background` **is** `#2f343e`, byte-identical to
> `elevated_surface.background`, so the dead step is **exactly 0.0000**, not "+0.2".

The current ladder, re-measured from the verified hexes, showing the dead step:

| | Zed One Dark | CIE L\* | **WCAG Y** | step (Y) |
|---|---|---|---|---|
| canvas (`background`) | `#282c33` | 17.88 | 0.0249 | — |
| `surface.background` | `#2f343e` | 21.61 | 0.0341 | +0.0092 |
| **`elevated_surface.background`** | `#2f343e` | 21.61 | 0.0341 | **0.0000 ← dead** |

*(`element.background` `#2e343e` — L\* 21.52, Y 0.0338 — is a different token and is not a
rung; it appeared in the v1 table by mistake.)*

Proposed model — four levels, separation by step and hairline **first**. **The `bg step`
column is normative and is in WCAG Y**; the L\* figure is informational only:

| Level | Role | bg step (dark) | Border | Shadow | Rim (inset) |
|---|---|---|---|---|---|
| `e0` | editor / canvas | base | none | none | none |
| `e1` | panel, sidebar, status bar | **≥ +0.010 Y** (≈ +4.5 L\*) | 1px hairline | **none** | none |
| `e2` | popover, dropdown, hover card | **≥ +0.012 Y** (≈ +4 L\*) | 1px hairline | `0 2px 3px /12%` + `0 1px 0 /6%` | `inset 0 1px 0 white/5%` |
| `e3` | modal, command palette | **≥ +0.014 Y** (≈ +4 L\*) | 1px border | Zed's existing 4-layer `ModalSurface` stack | `inset 0 1px 0 white/6%` |

As shipped, Zeo Dark clears every rung: `#2d2a37` → `#363443` → `#403d4f` → `#49465b`,
steps **+0.0116 / +0.0135 / +0.0156 Y**.

### Borders travel with the ladder *(v2 — added after validation caught the regression)*

**The border tokens must sit *outside* the surface ladder, never inside it.** This is not a
palette nicety; it is the other half of R2.1, and 5.1 got it wrong.

Upstream One Dark's ladder is **flat** (`surface` == `elevated_surface`, finding 2), so a
single fixed `border.variant` sat consistently above all of it. **Zeo introduced the ladder
and left the border where it was** — so `border.variant` `#403d4d` ended up *lighter* than
e1, **byte-adjacent to e2** (`#403d4f` — a contrast of **1.00:1**), and *darker* than e3. The
hairline crosses the ladder instead of clearing it, and collapses to nothing exactly at the
elevated surface.

This matters more than a normal palette bug because **the chokepoint paints `border_variant`
on every elevated surface** (`styled_ext.rs:43` — see component 4). So the invisible hairline
is not confined to one widget: it is every picker, palette, popover, context menu and toast,
i.e. all 71 call sites the story re-skinned. R2.1 demands *step **and** hairline*; what
shipped has only the step.

The floors are R3.10: **`border` ≥1.5:1** and **`border.variant` ≥1.25:1** against **every**
rung (`background`, `surface`, `elevated_surface`, `modal_surface`). Two floors, not one —
a single floor forces both tokens to the same colour and destroys the prominence hierarchy.
Both floors beat what they replace (upstream One: 1.43:1 / 1.12:1) while staying inside
principles 1 and 10 (a *low-contrast*, often translucent hairline — this is emphatically
**not** a 3:1 boundary; see the note under R3.10).

**Story 003 inherits this rule.** It touches the chrome containers, which sit on this same
ladder — any rung it adds or moves must re-verify the border floors, or it will reintroduce
exactly this defect one surface at a time.
...
- **Contrast floor** *(rewritten at v2 — the v1 wording is what shipped unasserted)*:
  - the accent clears WCAG AA (≥4.5:1) against the editor background in both variants (R3.5);
  - UI text clears AA;
  - the **focus ring** (`border.focused`) clears 3:1 against every surface (R3.9) — this,
    and not the surface separators, is what WCAG 1.4.11 governs;
  - **`border` ≥1.5:1 and `border.variant` ≥1.25:1 against every surface rung** (R3.10).

  All four are computable directly from the theme JSON — no rendering needed, which is
  precisely why there was no excuse for the v1 clause ("boundaries clear 3:1") never being
  written as a test. **It was demanded here, dropped in transcription to `tasks.md`, and
  the palette shipped with a 1.00:1 hairline.** A requirement stated only in the design and
  never in a task is a requirement that does not exist.
- **Token coverage:** a regression grep — zero `.rounded_*()` literals remain in the
  ...
  **GOTCHA: the grep must be per-corner-aware.** `\.rounded_(xs|sm|md|...)\(\)` does **not**
  match `.rounded_tl_md()` and friends — which are exactly what `button_like.rs` used. The
  naïve pattern returns 0 against a file still full of literals. Use
  `\.rounded_((tl|tr|br|bl)_)?(xs|sm|md|lg|xl|2xl|3xl|full)\(\)`.
...
| Row | Pass condition |
|---|---|
| Contrast | accent ≥4.5:1 on editor bg, both modes (R3.5); focus ring ≥3:1 (R3.9); `border` ≥1.5:1 and `border.variant` ≥1.25:1 against every surface rung (R3.10). All checkable from the theme JSON, and on the rendered shots with `magick` |
| Token coverage | zero radius literals in converted files — grepped with the **per-corner-aware** pattern; every converted surface reads the token |
